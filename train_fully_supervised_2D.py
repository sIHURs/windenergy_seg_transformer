...
def train(args, snapshot_path):
    ...
    if args.model == 'unet':
        ...
        print("train swin unet")
        model = ViT_seg(config, img_size=args.patch_size, num_classes=num_classes).cuda()
    ...
    db_train = BaseDataSetsDTU(base_dir=args.root_path, split="train", supervised=True, transform=transforms.Compose([
        RandomGenerator(args.patch_size)]))

    db_val = BaseDataSetsDTU(base_dir=args.root_path, split="val", transform=transforms.Compose([ResizeGenerator(args.patch_size)]))    
    ...
    for epoch_num in iterator:
        for i_batch, sampled_batch in enumerate(trainloader):
            ...
            # resize back to 1003x1003
            outputs = F.interpolate(outputs, size=(1003,1003), mode='bilinear', align_corners=False)
            outputs_soft = F.interpolate(outputs_soft, size=(1003, 1003), mode='bilinear', align_corners=False)
        
            # assert outputs.shape == (batch_size, 2, 1003, 1003)
            # assert outputs_soft.shape == (batch_size, 2, 1003, 1003)
            ...
            if iter_num > 0 and iter_num % 100 == 0:
                ...
                evaluation_list.append([iter_num, performance, mean_hd95, mean_iou])

                if mean_iou > best_performance:
                ...
                evaluation_df = pd.DataFrame(evaluation_list, columns=['Iteration', 'Performance', 'Mean_HD95', 'Mean_IOU_'])
